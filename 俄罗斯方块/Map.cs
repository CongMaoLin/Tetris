    ...
    internal class Map:IDraw
    {
        ...
        public void IfRowsFull()
        {
            ...
            foreach(DrawObject obj in D_Walls)
            {
                ...
                {
                    if(obj.positon.y == Game.height - i-2)
                    {
                        RowsCounts[i] += 1;
                    }
                }
            ...
            for(int i=0;i<RowsCounts.Length;i++ )
            {
                ...
                else if (RowsCounts[i] == (Game.width-14)/2 - 2)
                {
                    ...
                    {
                        if(D_Walls[j].positon.y == Game.height - 2)
                        {
                            continue;
                        }
                        else if (D_Walls[j].positon.y == Game.height -i - 2)
                        {
                            D_Walls[j].Clear();
                            D_Walls.RemoveAt(j);
                            RowsCounts[i]--;
                    ...
                    for (int k = 0; k < D_Walls.Count; k++)
                    {
                        ...
                        if(D_Walls[k].positon.y >= Game.height - i - 2)
                        {
                            continue;
                        }
                        else { D_Walls[k].positon.y += 1; }
                    }
                }
            }
